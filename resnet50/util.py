...
def compute_mean_iou(pred, label):
    ...
    I = np.zeros(num_unique_labels-1)
    U = np.zeros(num_unique_labels-1)
    ...
    for index, val in enumerate(unique_labels):
        if val!=19:
            ...
            for i in range(len(pred)):
                if pred[i]==val or label[i]==19:
                    pred_i.append(True)
                else:
                    pred_i.append(False)
                if label[i]==val or label[i]==19:
                    label_i.append(True)
                else:
                    label_i.append(False)
...
def evaluate_segmentation(pred, label, num_classes):
